...
public class ManipulatorHelper {
     ...
     * 
     * @param context - Any context object. Required for the
     *            ContentProviderHelper, because Android for some reason
     *            requires a context to be able to get access to content
     *            providers.
     */
    public void calculate(Context context) {
        SharedPreferences prefs = context.getSharedPreferences("ntnu.stud.valens.contentprovider",
                Context.MODE_PRIVATE);
        long lastUpdateTime = prefs.getLong("lastUpdateTime", 0);
        Calendar c = Calendar.getInstance();
        c.setTimeInMillis(lastUpdateTime);
        if (!(c.get(Calendar.DATE) == Calendar.getInstance().get(Calendar.DATE)) || lastUpdateTime == 0) {
            ...
            if (!(Double.isNaN(gaitParameters[0]) || Double.isNaN(gaitParameters[1]))) {
     ...
     * 
     * @param steps - The list of steps that gait parameters should be
     *            calculated from.
